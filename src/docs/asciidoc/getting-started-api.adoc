...
As described in the section about authenticating, in order to successfully issue a call against Boclips API, you need to retrieve our access token and send it over in the `Authorization` header of any subsequent HTTP requests.

[source,javascript]
const response = await axios.get('https://api.boclips.com/v1/videos', {
  headers: {
