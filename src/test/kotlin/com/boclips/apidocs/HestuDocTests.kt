...
class HestuDocTests : AbstractDocTests() {
    ...
    val versionHeaders = HeaderDocumentation.requestHeaders(
        HeaderDocumentation
            .headerWithName("X-Boclips-Api-Version")
            .optional()
            .description("Specify a version of this Endpoint in YYYY-MM-DD format. We strongly advise sending this header to protect against breaking changes")
    ...
    fun `Get highlights`(restDocumentation: RestDocumentationContextProvider) {
        ...
        RestAssured.given(hestuDocumentationSpec)
            ...
            .`when`()
            .get("$HIGHLIGHTS_PATH?query=test").apply { prettyPrint() }
            .then()
            .assertThat().statusCode(CoreMatchers.`is`(200))
    }
}
