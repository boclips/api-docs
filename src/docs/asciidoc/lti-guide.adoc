...
There is plenty of detailed documentation available on LTI (some of it linked below), but in a nutshell an LTI workflow looks like this:
...
=== Anatomy of a Lunch Request

[[lti-v1p1-lti-parameters]]
==== LTI Parameters

A minimal subset LTI launch request parameters required to access Boclips resources is presented in the table below.
...
Below is an example of a valid LTI launch request issued using:
