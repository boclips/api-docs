...
This works by generating a unique access token for each user. To achieve that, you can manually add a user id when requesting a token via the `client_credentials flow` by including a header:
...
operation::client-credentials-personalised-example[snippets='http-request,curl-request,request-parameters']
