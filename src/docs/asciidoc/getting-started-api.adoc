...
Now let's see how to put this together and show a playable video to your users! Below is a minimal example that you can copy to get yourself started quickly.
...
<1> _Load player sources_
<2> _Specify the container to render the player in_
<3> _Configure a factory function that returns Boclips API tokens_
<4> _Load the video into the player_
