...
The Boclips API authorization is based on https://openid.net/connect/[OpenID Connect].
...
Once ready, we'll send you across the URL that will trigger the flow so you can retrieve an `access_token`.
...
. Your application extracts the temporary code and makes a background REST request to our authentication server
